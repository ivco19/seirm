...
for(AAA in seq(1.0  , 50,len=11))
for(GAM in seq(3.0  , 50,len=11))
for(MUU in lseq(0.001,1. ,5))
for(Eini in lseq(1,1000,5))
for(Iini in lseq(1,1000,5))
	{
	...
	if(case == 1){ 
		...
		out <- rk(xstart, times, SEIR, parms, hini = 1, method = "rk4")
		IT=out[,'I0']
		RT=out[,'R0']
		MT=out[,'M0']
	}
...
#porcent=c(0.304,0.430,0.266)
		...
		out <- rk(xstart, times, SEIR_3mod, parms, hini = 1, method = "rk4")
...
if(missing(Par_pref)){
	...
	if(case == 1)  	ddo(par=Par_pref,xstart=Ini_pref)
	else  		ddo3(par=Par_pref,xstart=Ini_pref)
