...
function(par,xstart){
   ...
   if(missing(par)){
	...
	simu=tryRK(parms=par,xstart=xstart)  #<- integro
...
function(par,xstart){
   ...
   source('src/load_data.R')
   if(missing(par)){
	simu=tryRK_3()  #<- integro
   }else
	simu=tryRK_3(parms=par,xstart=xstart)  #<- integro
#cargamos los datos reales
   Real=load_data(download=FALSE)
   iday=24 		#elijo dia 20 porque ya hay un buen nuemero de casos
   fday=end(Real[,1])[1]
   RInf=Real[,4][iday:fday]
   RMue=Real[,1][iday:fday]
   RRec=Real[,3][iday:fday]
